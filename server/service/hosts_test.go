...
func TestSetHostDeviceMapping(t *testing.T) {
	...
	t.Run("IDP source success with valid SCIM user", func(t *testing.T) {
		...
		userCtx := test.UserContext(ctx, test.UserAdmin)
		result, err := svc.SetHostDeviceMapping(userCtx, 1, "user@example.com", DeviceMappingIDP)
		require.NoError(t, err)
	...
	t.Run("mdm_idp_accounts source success with valid SCIM user", func(t *testing.T) {
		ds := new(mock.Store)
		svc, ctx := newTestService(t, ds, nil, nil, &TestServerOpts{License: &fleet.LicenseInfo{Tier: fleet.TierPremium}})
		...
		userCtx := test.UserContext(ctx, test.UserAdmin)
		result, err := svc.SetHostDeviceMapping(userCtx, 1, "user@example.com", fleet.DeviceMappingMDMIdpAccounts)
		require.NoError(t, err)
		require.True(t, ds.SetOrUpdateIDPHostDeviceMappingFuncInvoked)
		require.True(t, ds.SetOrUpdateHostSCIMUserMappingFuncInvoked) // Should be called since SCIM user exists
		require.NotNil(t, result)
	...
	t.Run("IDP source success with any username when SCIM user not found", func(t *testing.T) {
		...
		userCtx := test.UserContext(ctx, test.UserAdmin)
		result, err := svc.SetHostDeviceMapping(userCtx, 1, "any@username.com", fleet.DeviceMappingMDMIdpAccounts)
		require.NoError(t, err)
		require.True(t, ds.SetOrUpdateIDPHostDeviceMappingFuncInvoked)
	...
	t.Run("IDP source fails without premium license", func(t *testing.T) {
		...
		userCtx := test.UserContext(ctx, test.UserAdmin)
		_, err := svc.SetHostDeviceMapping(userCtx, 1, "user@example.com", DeviceMappingIDP)
		require.Error(t, err)
		assert.Equal(t, fleet.ErrMissingLicense, err)
	})

	t.Run("mdm_idp_accounts source fails without premium license", func(t *testing.T) {
		ds := new(mock.Store)
		svc, ctx := newTestService(t, ds, nil, nil, &TestServerOpts{License: &fleet.LicenseInfo{Tier: fleet.TierFree}})
		...
		userCtx := test.UserContext(ctx, test.UserAdmin)
		_, err := svc.SetHostDeviceMapping(userCtx, 1, "user@example.com", fleet.DeviceMappingMDMIdpAccounts)
		require.Error(t, err)
		assert.Equal(t, fleet.ErrMissingLicense, err)
	})
	...
	t.Run("invalid source returns validation error", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "must be 'custom' or 'idp' or 'mdm_idp_accounts'")
